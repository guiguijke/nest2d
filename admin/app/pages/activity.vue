...
function fmtDate(d: any) {
  return d ? new Date(d).toLocaleString('fr-FR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) : '—'
}
  ...
  <div class="space-y-5">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <select v-model.number="windowDays" class="input w-28">
    ...
    <template v-else-if="data">
      ...
      <div class="card">
        <p class="text-sm text-ink-100">
          ...
          ({{ fmtDurationMin(data.jobsToday.totalTimeMin) }} de calcul),
          <span class="font-semibold text-white">{{ data.payments.creditPayments }}</span> achat(s) de crédits
          et <span class="font-semibold text-white">{{ data.payments.newSubscriptions }}</span> nouvelle(s) souscription(s).
        </p>
      </div>
      ...
      <div class="grid gap-4 md:grid-cols-2">
        ...
        <div class="card space-y-3">
          ...
          <p v-if="data.jobsToday.avgDensity" class="text-[11px] text-ink-400">Densité moyenne nesting : {{ Math.round(data.jobsToday.avgDensity * 100) }}%</p>
        </div>

        <!-- Activity pulse -->
        <div class="card space-y-3">
          <h2 class="text-sm font-semibold">Pic d'activité (par heure)</h2>
          <div v-if="data.pulse.length" class="flex items-end gap-[2px]" style="height: 56px">
            <div
          ...
          <p v-else class="text-[11px] text-ink-400">Aucune activité enregistrée sur la période.</p>
        </div>
      </div>
