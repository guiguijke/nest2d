...
export default defineEventHandler(async (event) => {
  ...
  const signups = await db
    ...
    .find({ createdAt: { $gte: since } }, { projection: { _id: 0, id: 1, name: 1, email: 1, provider: 1, signupCountry: 1, createdAt: 1 } })
    .sort({ createdAt: -1 })
    .limit(50)
    .toArray()
  ...
  const [classicJobs, stripJobs] = await Promise.all([
    db
      ...
      .aggregate([
        { $match: jobMatch },
        { $group: { _id: null, count: { $sum: 1 }, totalTimeMin: { $sum: { $ifNull: ['$timeTaken', 0] } }, avgDensity: { $avg: { $ifNull: ['$density', null] } } } },
      ])
      .toArray(),
    db
  ...
  // ---- Payments today ----
  const creditPayments = await db.collection(COL.transactions).countDocuments({
    status: 'completed',
    updatedAt: { $gte: since },
  })
  ...
  return {
    ...
    signups,
    jobsToday,
    payments: { creditPayments, newSubscriptions },
    pulse,
  }
})
