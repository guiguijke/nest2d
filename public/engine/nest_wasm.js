 ...
 * @returns {string}
 */
export function run_nesting(instance_json, params_json, seed) {
    let deferred4_0;
    let deferred4_1;
    try {
        ...
        const ret = wasm.run_nesting(ptr0, len0, ptr1, len1, seed);
        var ptr3 = ret[0];
        var len3 = ret[1];
        if (ret[3]) {
            ptr3 = 0; len3 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('nest_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
