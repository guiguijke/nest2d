...
| SVG | **usvg 0.48** + sampler maison, **chunk WASM lazy** (chargé au premier SVG — le bundle principal = moteur + parser DXF, ~450 Ko gz ; budget total +2 Mo gz) | usvg = référence maintenue, pur Rust, gère units/viewBox/transforms. Le sampling courbes réplique svgelements (length/step/clamp) — risque parité connu (§4.3), à verrouiller au harnais SVG. |
| Booléens (canaux) | **i_overlay** (déjà vendorisé via jagua→geo-buffer) — **mais ring-splice = CANDIDAT PRINCIPAL (amendement 2026-08-06)** | Splice = recoller les anneaux au plus proche (arithmétique pure, zéro booléen, topologiquement équivalent au canal capillaire). Les deux sont prototypés ; le harnais tranche sur parité + qualité (holesFilled, canaux utilisables D-MOT-2). Si le splice gagne, la dépendance aux booléens s'allège. |
