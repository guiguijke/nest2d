...
| **T2 (+2-3 sem.)** | Lot 4 (P4 worker finalisation, P5 plateau SPP, P6 zones //) ; **3.2 robustesse import (démarrage)** ; **3.1 calques + identifiants** (~1 sem., les arcs sont déjà conservés) ; **AF5 (vérif L3-bis)** : expiration serveur des jobs `awaiting_local` orphelins (~10 min sans prise en charge → `cancelled`, carte « non pris en charge par cet appareil ») — un orphelin bloque l'utilisateur en 409 jusqu'à annulation manuelle | Mono-tôle 95-145 → 20-40 s ; rapport de réparation par fichier (v1) ; plus d'orphelins awaiting_local |
