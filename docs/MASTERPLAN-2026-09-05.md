...
|---|---|---|
| **T0 (semaine en cours)** | ~~décision k + SAMPLE_CFG~~ **FAIT 05/09** (k=3/plancher 30/sans plancher temps ; SAMPLE_CFG inchangé) ; ~~lot 2 (P3 + P7)~~ LIVRÉ + **corrigé L2-bis 05/09 nuit — non déployé** (A/B : chute Compaction restaurée à ±5 mm via compression du schedule à 200 it. ; AB2 observabilité des alternatives écartées ; `RAPPORT-PERF-UX-L2-BIS-2026-09-05.md`) ; webhook Stripe **TOUJOURS À VÉRIFIER** ; campagne feedback à envoyer | Job standard ≤ 25 s ✓ ; paiements vivants ⏳ |
