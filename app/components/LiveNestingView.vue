...
// (stability). SPP nuance: phase-2 frames (mapped back by the engine) live
// at width ≈ phase-1 champion + engine slack (corridor = phase1 + 1 mm) —
// inside that window the tie is broken on used_height, the merge's
// secondary criterion. Without it the champion ignored the whole
// height-compaction phase (width ~constant, height dropping).
const SPP_TIE_WINDOW_MM = 1.0;
...
function isBetter(a, b) {
    ...
    // recuit) est LE critère qui progresse — sans lui la vue ignore toutes
    // les améliorations du plateau et fige (« 1 maj et c'est tout »,
    // constat user 2026-08-29).
    ...
    // Égalité parfaite : BPP accepte la frame fraîche (rotation entre les
    // incumbents des walks à ~1 Hz — sans ça la vue fige sur le premier
    // layout pendant tout le plateau du recuit, « 1 maj et c'est tout »,
    // constat user 2026-08-29) ; SPP garde l'incumbent (stabilité du
    // corridor phase 2).
